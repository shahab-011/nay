...
function Column({ stage, leads, onDragStart, onDrop, onCardClick }) {
  ...
  return (
    <div
      ...
      style={{ minWidth: 240, flex: '0 0 240px', background: over ? '#F5F3FF' : '#F3F4F6', borderRadius: 14, padding: '12px 10px', border: over ? '2px dashed #7C3AED' : '2px solid transparent', transition: 'all 150ms', maxHeight: '70vh', overflowY: 'auto' }}
    >
...
function NewLeadModal({ onClose, onSave }) {
  const [form, setForm] = useState({ name: '', email: '', phone: '', practiceArea: 'Family Law', source: 'Website', estimatedValue: '', stage: 'New Lead', description: '' });
  ...
  const submit = async () => {
    ...
    setSaving(true);
    try {
      const r = await leadsApi.create({ ...form, estimatedValue: Number(form.estimatedValue) || 0 });
    ...
    <div style={{ position: 'fixed', inset: 0, zIndex: 200, display: 'flex', alignItems: 'center', justifyContent: 'center', padding: 20, background: 'rgba(11,11,20,0.55)', backdropFilter: 'blur(4px)' }}>
      <motion.div initial={{ opacity: 0, scale: 0.96 }} animate={{ opacity: 1, scale: 1 }} style={{ background: '#fff', borderRadius: 20, width: '100%', maxWidth: 560, maxHeight: '90vh', overflowY: 'auto', padding: 28 }}>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 14 }}>
          {[['name', 'Full Name / Company', 'text'], ['email', 'Email', 'email'], ['phone', 'Phone', 'tel'], ['estimatedValue', 'Estimated Value (PKR)', 'number']].map(([k, pl, type]) => (
            <div key={k}>
        ...
        <div style={{ marginTop: 14 }}>
          ...
          <textarea value={form.description} onChange={e => set('description', e.target.value)} style={{ ...inp, height: 80, resize: 'vertical' }} placeholder="Details about the lead…" />
        </div>
...
function LeadDetail({ lead, onClose, onUpdate, onConvert }) {
  ...
  return (
    <motion.div
      ...
      style={{ position: 'fixed', right: 0, top: 0, bottom: 0, width: 360, zIndex: 150, background: '#fff', borderLeft: '1.5px solid #E5E7EB', boxShadow: '-8px 0 32px rgba(0,0,0,0.08)', overflowY: 'auto', padding: 24 }}
    >
      ...
      <div style={{ display: 'flex', flexDirection: 'column', gap: 14 }}>
        {[
          ...
          ['Value', lead.estimatedValue ? `PKR ${lead.estimatedValue?.toLocaleString()}` : '—', I.DollarSign],
        ...
        <div>
          ...
          <div style={{ display: 'flex', flexWrap: 'wrap', gap: 6 }}>
            {STAGES.map(s => {
              ...
              return (
                <button key={s} onClick={() => handleStage(s)} style={{ padding: '5px 12px', borderRadius: 20, border: `1.5px solid ${lead.stage === s ? sc.text : '#E5E7EB'}`, background: lead.stage === s ? sc.bg : '#fff', color: lead.stage === s ? sc.text : '#6B7280', cursor: 'pointer', fontSize: 11, fontWeight: 700 }}>{s}</button>
              );
            })}
          </div>
        </div>

        {!lead.convertedToMatterId && (
          <button onClick={handleConvert} disabled={converting} style={{ width: '100%', padding: '11px', borderRadius: 10, background: '#059669', color: '#fff', border: 'none', cursor: 'pointer', fontSize: 13, fontWeight: 700, display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 8, opacity: converting ? 0.7 : 1 }}>
            <I.Briefcase size={15} /> {converting ? 'Converting…' : 'Convert to Matter'}
          </button>
        )}
        {lead.convertedToMatterId && (
          <div style={{ background: '#ECFDF5', borderRadius: 10, padding: 12, fontSize: 13, color: '#059669', fontWeight: 700, textAlign: 'center' }}>
            ✓ Converted to Matter
          </div>
        )}
      </div>
...
export default function LeadPipeline() {
  ...
  function updateStage(id, stage) {
    setLeads(ls => ls.map(l => l._id === id ? { ...l, stage } : l));
    setSelected(s => s && s._id === id ? { ...s, stage } : s);
  }

  function handleConverted(result) {
    const matterId = result.matter?._id || result._id;
    setLeads(ls => ls.map(l => l._id === selected._id ? { ...l, stage: 'Won', convertedToMatterId: matterId } : l));
    setSelected(null);
  }
    ...
    <div style={{ paddingTop: 80, minHeight: '100vh', background: '#F8F9FC' }}>
      <div style={{ maxWidth: 1400, margin: '0 auto', padding: '32px 24px 60px' }}>
        ...
        <div style={{ display: 'flex', gap: 12, overflowX: 'auto', paddingBottom: 12 }}>
          {STAGES.map(stage => (
            <Column key={stage} stage={stage} leads={leads.filter(l => l.stage === stage)} onDragStart={handleDragStart} onDrop={handleDrop} onCardClick={setSelected} />
          ))}
        </div>
      </div>
      ...
      <AnimatePresence>
        {selected && (
          <>
            ...
            <LeadDetail lead={selected} onClose={() => setSelected(null)} onUpdate={updateStage} onConvert={handleConverted} />
          </>
        )}
      </AnimatePresence>
    </div>
